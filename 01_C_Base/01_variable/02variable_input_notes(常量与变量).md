...
//	变量的定义与输入 定义：int a; 在内存中申请空间存储整数。
//输入：使用 scanf("%d %d", &a, &b); 读取用户输入。
//&（取地址符）：这是关键。scanf需要知道变量存放在内存的哪个位置，所以必须传入变量的地址（&a），而 printf 只需要变量的值。
//格式化：%d 表示读取整数。两个 %d 之间用空格隔开，表示输入时也用空格分隔两个数字。
//	 
#include <stdio.h>
